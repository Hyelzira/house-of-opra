...
// Removed: const Dad = "../assets/image/edu.png"; - Not needed with ASSET_PATH
...
interface ProgramItem {
    title: string;
    icon: string; // This now holds the full path
}
...
const ProgramModal: React.FC<ProgramModalProps> = ({ item, onClose }) => {
    ...
    // Detailed explanations (remains the same)
            ...
            >
                ...
                <div className="flex flex-col items-center text-center">
                    ...
                    <div className="w-20 h-20 flex items-center justify-center rounded-full bg-purple-200 mb-6 overflow-hidden border-4 border-purple-400">
                        {/* Image source is now the full asset path */}
                            ...
                            {programDetails.focus.map((f: string, index: number) => (
                                <li key={index} className="flex items-start">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-purple-500 mr-2 shrink-0" viewBox="0 0 20 20" fill="currentColor">
                                        <path fillRule="evenodd" d="M10 18a8 8 010-16 8 8 010 16zm-3.707-9.293a1 1 0 011.414 0L10 10.586l2.293-2.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clipRule="evenodd" />
                                    </svg>
                                    {f}
                                </li>
                            ))}
...
const HomeAbout = () => {
    ...
    // ✨ UPDATED: Each item now has a unique asset path appended to ASSET_PATH
    const items = [
        {
                ...
                {items.map((item, i) => (
                    ...
                    >
                        <div className="w-16 h-16 mx-auto flex items-center justify-center rounded-full bg-purple-200 mb-4 overflow-hidden shadow-inner">
                            {/* Image source is now the unique asset path */}
                            <img src={item.icon} alt={item.title} className="w-full h-full object-contain" />
                        </div>
